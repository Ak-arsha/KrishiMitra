...
export default function MarketIntelligencePage() {
  ...
  const loadFeed = async () => {
    ...
    try {
      ...
      setFeed(feedData);
      if (feedData.length > 0 && !selectedCrop) {
        openCropSummary(feedData[0].crop);
      }
    } catch (e: any) {
      console.warn("API market feed fallback engaged:", e);
      const defaultCrops = [
      ...
      const fallbackFeed = defaultCrops.map((c) => ({
        crop: c.crop,
